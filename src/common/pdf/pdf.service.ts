...
export class PdfService {
  ...
  private generateTable(
    ...
    const generateLine = (isSmall: boolean): string => {
    ...
    const body: TableCell[][] = [
      ...
      [
        {
          text: generateLine(isSmallWidth),
          style: 'styleCenter',
  ...
  async generateInscriptionPdf(inscription: InscriptionSelectType): Promise<Buffer> {
    ...
    const content: TDocumentDefinitions['content'] = [
        ...
        table: {
          ...
          body: [
            [
              ...
              this.generateTable(
                'Edad',
                `${this.calculateAge(inscription.student!.birthdate)} años`,
                true,
              ),
              this.generateTable(
                'Fecha de nacimiento',
              ...
              this.generateTable('Sexo', `${inscription.student!.gender}`, true),
            ],
          ],
        },
      },
      ...
      inscription.assignmentRooms.map(assignmentRoom => ({
        ...
        table: {
          ...
          body: [
            [
              this.generateTable('Programa', `${assignmentRoom.room.specialty.name}`), // Cambia esto según tu lógica
              this.generateTable(
                'Inicio de sesión',
                `${format(assignmentRoom?.start ?? new Date(), 'dd-MMMM-yyyy', { locale: es })}`
              ),
              this.generateTable('Módulo', `1er módulo`),
              this.generateTable('Sesiones', `${assignmentRoom?.room.specialty.branchSpecialties[0].numberSessions}`, true),
            ],
          ],
        },
      })),
    ...
    const docDefinition: TDocumentDefinitions = {
      ...
      content,
      defaultStyle: {
        font: 'Poppins',   // Aquí indicas la fuente que definiste
        fontSize: 12,
      },
      styles: {
        stylePagrafer: {
