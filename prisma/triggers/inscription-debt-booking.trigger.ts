...
export async function createDebtOnBookingTrigger() {
  ...
  await prisma.$executeRawUnsafe(`
    ...
    BEGIN
        IF NEW.booking_id IS NOT NULL AND NEW.student_id IS NULL THEN
            ...
            -- Crear la deuda
            INSERT INTO "debts" (
                inscription_id, total_amount, remaining_balance, type, created_by
            ) VALUES (
                NEW.id, booking.amount, booking.amount, 'BOOKING', NEW.created_by
            )
            ...
            INSERT INTO "payments" (
                debt_id, invoice_id, reference, amount, created_by
            ) VALUES (
                debt_id, invoice_id, 'PAGO AUTOMATICO', booking.amount, NEW.created_by
            );
        END IF;
